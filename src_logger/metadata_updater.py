...
logger.info("Looking to update metadata for model")
...
# Paths
    ...
    if metadata_file.exists():
        ...
        if new_model_info["r2_score"] > old_r2:
            logger.info(f"New model R² ({new_model_info['r2_score']}) is better than old ({old_r2})")
            # Update metadata
            ...
            logger.info(f" New model R² ({new_model_info['r2_score']}) is worse. Keeping old model")
    else:
        # No metadata exists → save new
        logger.info("ℹ️ No existing metadata. Saving new model and metadata.")
        with open(metadata_file, "w") as f:
            json.dump(new_model_info, f, indent=4)

except Exception as e:
    logger.exception("Error occurred while updating metadata.")
    raise